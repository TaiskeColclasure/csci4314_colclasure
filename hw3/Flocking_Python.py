    ...
    def flocking(self):
        ...
        for i in range(0, self.frames):
            ...
            for n in range(0, self.N):
                ...
                if np.linalg.norm(self.v[:,:-1]) > self.vlimit:
                    self.v[:,:-1] = self.v[:,:-1] / np.linalg.norm(self.v[:,:-1], axis=0)
                self.v[:,-1] = np.array([0,0])

            # Update position
            self.p[:,:-1] += self.v[:, :-1] * self.timestepsize
            ...
            plt.savefig('bruh{}.png'.format(i))
            plt.pause(0.005)


n_agents = 100
c1 = 0.0001  # attraction
c2 = 0.01  # repulsion
c3 = 1.0  # heading
c4 = 0.3  # randomness
...
frames = []
for i in range(500):
    image = imageio.v2.imread('bruh{}.png'.format(i))
    frames.append(image)
imageio.mimsave('obstical.gif', frames)
